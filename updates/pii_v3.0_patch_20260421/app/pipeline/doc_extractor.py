"""
문서 파일(Word/PDF/Excel/PowerPoint/XML)에서 텍스트 라인 추출
- .docx : python-docx (단락 + 표 셀)
- .pdf  : pdfplumber (텍스트 레이아웃 유지)
- .xlsx/.xls : openpyxl (각 행 → 탭 구분 텍스트)
- .pptx/.ppt : zipfile + DrawingML XML 파싱 (python-pptx 불필요)
...
SUPPORTED_DOC_EXTENSIONS = {'.docx', '.pdf', '.xlsx', '.xls', '.xlsm', '.pptx', '.ppt', '.xml'}
...
def _extract_excel(path: Path) -> Iterator[tuple[int, str]]:
    try:
        if not zipfile.is_zipfile(str(path)):
            print(f"  [Excel 오류] {path.name}: 유효한 xlsx 파일이 아닙니다."
                  f" 구형 .xls 바이너리 형식이면 Excel에서 .xlsx로 다시 저장하세요.")
            return
        import openpyxl
...
def _extract_pptx(path: Path) -> Iterator[tuple[int, str]]:
    """
    .pptx / .ppt 파일에서 텍스트 추출.
    pptx는 zip 구조로 DrawingML XML을 직접 파싱 (python-pptx 불필요).
    구형 .ppt 바이너리는 zip 구조가 아니므로 안내 메시지 출력.
    """
    _DRAWING_NS = 'http://schemas.openxmlformats.org/drawingml/2006/main'

    if not zipfile.is_zipfile(str(path)):
        print(f"  [PPT 오류] {path.name}: 구형 .ppt 바이너리 형식은 지원하지 않습니다."
              f" PowerPoint에서 .pptx로 다시 저장하세요.")
        return
    ...
    try:
        ...
        with zipfile.ZipFile(str(path), 'r') as zf:
            ...
            if not slide_files:
                print(f"  [PPTX 오류] {path.name}: 슬라이드를 찾을 수 없습니다.")
                return

            line_no = 1
            for slide_path in slide_files:
                slide_num = slide_path.split('slide')[-1].replace('.xml', '')
                yield line_no, f"[Slide: {slide_num}]"
                line_no += 1

                with zf.open(slide_path) as f:
                    root = ET.parse(f).getroot()

                # 단락 단위로 텍스트 수집 (<a:p> 내 <a:t> 합산)
                for para in root.iter(f'{{{_DRAWING_NS}}}p'):
                    texts = [
                        (t.text or '').strip()
                        for t in para.iter(f'{{{_DRAWING_NS}}}t')
                        if (t.text or '').strip()
                    ]
                    if texts:
                        yield line_no, ' '.join(texts)
                        line_no += 1

    except Exception as e:
        print(f"  [PPTX 오류] {path.name}: {e}")
