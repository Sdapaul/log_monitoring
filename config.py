...
PII_PATTERNS: dict[str, tuple[str, str]] = {
    ...
    # 구분자: 없음/하이픈/공백/공백+하이픈+공백 허용 (예: 850101-1234567, 850101 - 1234567)
    "RRN": (
        r"(?<!\d)\d{6}[ \t]{0,2}-?[ \t]{0,2}[1-9]\d{6}(?!\d)",
        "CRITICAL"
    ),
    # 한국 전화번호: 010-XXXX-XXXX, 02-XXX-XXXX 등
    # 구분자: 없음/하이픈/공백/공백+하이픈+공백 허용
    "PHONE": (
        r"\b0(?:1[016-9]|2|[3-9]\d)[ \t]{0,2}-?[ \t]{0,2}\d{3,4}[ \t]{0,2}-?[ \t]{0,2}\d{4}\b",
        "HIGH"
    ),
    # 계좌번호 (키워드 선행)
    "ACCOUNT_NO": (
